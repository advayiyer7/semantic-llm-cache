...
Per request:
  ...
  3. On a non-streaming miss, a single-flight lock collapses concurrent identical
     requests: the first calls the provider and stores; the rest re-check and
     reuse it.
  4. Streaming misses frame deltas as SSE while buffering the full text, then
     store it once the stream ends. (Streaming misses are NOT single-flighted —
     sharing one in-flight stream across clients isn't worth the slow-client
     lock-holding risk; concurrent identical streams each call the provider.)
...
async def chat_completions(request: Request):
    ...
    if cache_result is None:
        ...
        # Labelled BYPASS (not MISS) so the metric distinguishes degraded from cold.
        response = await _provider_complete(provider, req, key)
        return JSONResponse(response, headers={**base_headers, "X-Cache": "BYPASS"})
...
async def _stream_and_store(provider, req, cache, cache_result, conversation, ttl, key):
    ...
    try:
        async for delta in provider.stream(req):
            if not started:
                # Defer the opening role chunk until the provider actually responds,
                # so a provider error at the start doesn't follow a "valid" first frame.
        ...
        if not started:
            yield sse(chunk(cid, req.model, {"role": "assistant"}))
        # finish_reason in the standard field — SDK-safe, unlike a top-level error key.
        yield sse(chunk(cid, req.model, {}, finish_reason="error"))
        yield SSE_DONE
        return

    # Store before the terminal frames so a client disconnect at [DONE] can't
    # skip the cache write (the post-yield path may not run on GeneratorExit).
    text = "".join(buffer)
